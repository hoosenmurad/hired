import { redirect } from "next/navigation";
import Link from "next/link";
import { Button } from "@/components/ui/button";
import { getCurrentUser } from "@/lib/actions/auth.action";
import { getJobTargetsByUserId } from "@/lib/actions/job-target.action";
import { Plus, Briefcase, Building, Target } from "lucide-react";

const JobTargetsPage = async () => {
  const user = await getCurrentUser();
  if (!user) redirect("/sign-in");

  const jobTargets = await getJobTargetsByUserId(user.id);

  return (
    <div className="container mx-auto py-8">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold">Your Job Targets</h1>
        <Button asChild>
          <Link href="/job-targets/create">
            <Plus className="h-4 w-4 mr-2" />
            Add Job Target
          </Link>
        </Button>
      </div>

      {jobTargets.length === 0 ? (
        <div className="text-center py-12">
          <Target className="h-16 w-16 mx-auto text-gray-400 mb-4" />
          <h2 className="text-2xl font-bold mb-4">No Job Targets Yet</h2>
          <p className="text-gray-600 mb-6">
            Add job targets to get personalized interview questions tailored to
            specific roles.
          </p>
          <Button asChild>
            <Link href="/job-targets/create">
              <Plus className="h-4 w-4 mr-2" />
              Create Your First Job Target
            </Link>
          </Button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {jobTargets.map((jobTarget) => (
            <div
              key={jobTarget.id}
              className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow"
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center space-x-2">
                  <Briefcase className="h-5 w-5 text-blue-600" />
                  <h3 className="font-semibold text-lg truncate">
                    {jobTarget.title}
                  </h3>
                </div>
                <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">
                  {new Date(jobTarget.createdAt).toLocaleDateString()}
                </span>
              </div>

              <div className="flex items-center space-x-2 mb-4">
                <Building className="h-4 w-4 text-gray-500" />
                <p className="text-gray-700 font-medium">{jobTarget.company}</p>
              </div>

              <p className="text-gray-600 text-sm mb-4 line-clamp-3">
                {jobTarget.description}
              </p>

              {/* Required Skills */}
              <div className="mb-4">
                <h4 className="text-sm font-medium text-gray-700 mb-2">
                  Key Skills
                </h4>
                <div className="flex flex-wrap gap-1">
                  {jobTarget.requiredSkills.slice(0, 3).map((skill, index) => (
                    <span
                      key={index}
                      className="inline-block bg-blue-100 text-blue-800 px-2 py-1 rounded text-xs"
                    >
                      {skill}
                    </span>
                  ))}
                  {jobTarget.requiredSkills.length > 3 && (
                    <span className="inline-block bg-gray-100 text-gray-600 px-2 py-1 rounded text-xs">
                      +{jobTarget.requiredSkills.length - 3} more
                    </span>
                  )}
                </div>
              </div>

              {/* Responsibilities */}
              <div className="mb-6">
                <h4 className="text-sm font-medium text-gray-700 mb-2">
                  Key Responsibilities
                </h4>
                <ul className="text-sm text-gray-600 space-y-1">
                  {jobTarget.responsibilities
                    .slice(0, 2)
                    .map((responsibility, index) => (
                      <li key={index} className="flex items-start">
                        <span className="text-blue-500 mr-2">•</span>
                        <span className="line-clamp-1">{responsibility}</span>
                      </li>
                    ))}
                  {jobTarget.responsibilities.length > 2 && (
                    <li className="text-xs text-gray-500">
                      +{jobTarget.responsibilities.length - 2} more
                      responsibilities
                    </li>
                  )}
                </ul>
              </div>

              <div className="flex space-x-2">
                <Button asChild variant="outline" className="flex-1">
                  <Link href={`/job-targets/${jobTarget.id}`}>
                    View Details
                  </Link>
                </Button>
                <Button asChild className="flex-1">
                  <Link
                    href={`/onboarding/setup-interview?jobTargetId=${jobTarget.id}`}
                  >
                    Create Interview
                  </Link>
                </Button>
              </div>
            </div>
          ))}
        </div>
      )}

      {/* Quick Stats */}
      {jobTargets.length > 0 && (
        <div className="mt-12 bg-gray-50 rounded-lg p-6">
          <h2 className="text-xl font-semibold mb-4">Quick Stats</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-blue-600">
                {jobTargets.length}
              </p>
              <p className="text-sm text-gray-500">Total Job Targets</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-green-600">
                {[...new Set(jobTargets.map((jt) => jt.company))].length}
              </p>
              <p className="text-sm text-gray-500">Companies Targeted</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-purple-600">
                {
                  [...new Set(jobTargets.flatMap((jt) => jt.requiredSkills))]
                    .length
                }
              </p>
              <p className="text-sm text-gray-500">Unique Skills</p>
            </div>
          </div>
        </div>
      )}
    </div>
  );
};

export default JobTargetsPage;
